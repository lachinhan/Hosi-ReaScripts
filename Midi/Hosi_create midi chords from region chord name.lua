...
--Msg("region_cout")
--Msg(region_count)
  --commandID5 = reaper.NamedCommandLookup("_SWS_CREATETRK1")
  --reaper.Main_OnCommand(commandID5, 0) -- SWS: Create and select first track
    
  --reaper.Main_OnCommand(40042,0) -- Transport: Go to start of project  
  
  commandID2 = reaper.NamedCommandLookup("_S&M_GOTO_SEL_REGION1")
  reaper.Main_OnCommand(commandID2, 0) -- SWS/S&M: Go to/select region 1 (obeys smooth seek) 
  ...
  retval, isrgnOut, region_pos, region_end, region_name, region_index, region_color = reaper.EnumProjectMarkers3(0, regionidx)
  
  commandID3 = reaper.NamedCommandLookup("_RS558f5cec5230ef6366aae51c5f9138909bd8a01d")
  ...
  commandID1 = reaper.NamedCommandLookup("_RSa269108c0ba744d947b179c93a57c34399b55a43")
  reaper.Main_OnCommand(commandID1, 0) -- Script: ReaTrak Go to start of previous region.lua "_RSa269108c0ba744d947b179c93a57c34399b55a43"  
  --retval_region, _, region_pos, region_end, region_name, region_index, region_color = reaper.EnumProjectMarkers3(0, 0)
  media = reaper.GetSelectedMediaItem(0, 0)
  take = reaper.GetMediaItemTake(media, 0)
  length = reaper.BR_GetMidiSourceLenPPQ(take) -- length of region/midi item
  
  --take = reaper.MIDIEditor_GetTake(midieditor)
  
  --count loop selected items or move to next region in Action
...
--Msg("slash " ..slash)
  ...
  if not slash then slashnote = 0 end
  end

  octave = note1 - slashnote
  if octave > 12 and slashnote > 0 then slashnote = slashnote + 12 end
  
  --reaper.MIDI_InsertNote(take, true, false, 0, length -2, 1, note1, 50, false) -- Set root note1  
  note2 = 0
  note3 = 0
  note4 = 0
  note5 = 0
  ...
  reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1, 50, false) -- Set root note1  
  if slashnote > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, slashnote, 50, false) end -- chord note 2  
  if note2 > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1+note2, 50, false) end -- chord note 2
  if note3 > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1+note3, 50, false) end -- chord note 3 
  if note4 > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1+note4, 50, false) end -- chord note 4
  if note5 > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1+note5, 50, false) end -- chord note 5
  if note6 > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1+note6, 50, false) end -- chord note 6
  if note7 > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 0, note1+note7, 50, false) end -- chord note 7
  
--  if noteroot > 0 then reaper.MIDI_InsertNote(take, true, false, 0, length -2, 1, note1-noteroot, 50, false) end -- chord note 7
  
  --reaper.MIDI_DeleteNote(take, note1)  -- Delete set root note 
  --reaper.MIDI_Sort(take)
  
  commandID4 = reaper.NamedCommandLookup("_RSce47da3c9b1238de71cc94a1cb99732b9abd5e41")
  reaper.Main_OnCommand(commandID4, 0) -- Script: ReaTrak Go to start of next region.lua  
  ...
  --reaper.MIDIEditor_OnCommand(2,0) -- File: Close MIDI Editor window 2 
  ...
  --reaper.MIDIEditor_OnCommand(40214,0)-- Edit: Unselect all
  
  --commandID2 = reaper.NamedCommandLookup("_SN_FOCUS_MIDI_EDITOR")
  --reaper.Main_OnCommand(commandID2, 0) -- SWS/SN: Focus MIDI editor "_SN_FOCUS_MIDI_EDITOR"
  ...
  -- reaper.Main_OnCommand(40003,0) -- Edit: Select all notes 40003 
...
--Msg(buf)  
  ...
  --sel_note = reaper.MIDI_EnumSelNotes(take, noteidx)
  
  
  -- reaper.MIDI_SelectAll(take, true)  
  
    --hwnd = reaper.MIDIEditor_GetActive()
    --reaper.MIDIEditor_OnCommand( hwnd, 40002,0 ) -- Edit: Delete notes 40002
  
  --reaper.Main_OnCommand(40018,0)
  --commandID3 = reaper.NamedCommandLookup("_f4f1947be2fc8546ab52375e4281fbf2")-- RENAME Chord ACTION to REATRAK
  --reaper.MIDIEditor_OnCommand( hwnd,commandID3, 0) -- Custom: Chord:Maj

  
  --delete notes
  --Insert chord
  
  -- Count Midi Notes in Item and set length to region length
  --function CountNotes
  --count = reaper.FNG_CountMidiNotes(take)
--Msg("Take")
--Msg(take)
--Msg("Note Count")
--Msg(count)
...
--Msg("Number")
--Msg(i)
    --reaper.MIDI_SetNote(take, i, true, null, null, length, null, null, null, null)
    
    --reaper.Main_OnCommand(2,0)-- File: Close MIDI Editor window 2
    
    --Set MIDI note properties. Properties passed as NULL (or negative values) will not be set.   
    --end
  --end
