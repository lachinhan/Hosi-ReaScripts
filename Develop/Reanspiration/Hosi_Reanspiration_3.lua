  ...
  if visible then
    ...
    if reaper.ImGui_BeginTabBar(ctx, "MainTabBar") then
      ...
      if reaper.ImGui_BeginTabItem(ctx, "Generation") then
        ...
        changed_sec_dom, add_secondary_dominants = reaper.ImGui_Checkbox(ctx, "Add Secondary Dominants", add_secondary_dominants)
        if reaper.ImGui_IsItemHovered(ctx) then reaper.ImGui_SetTooltip(ctx, "50% chance to insert a V7 of the next chord (if it's not the tonic).") end
      ...
      if reaper.ImGui_BeginTabItem(ctx, "Creative Tools") then
        ...
        -- NEW WIDGETS (v3.1)
        changed, selected_melody_contour = reaper.ImGui_Combo(ctx, "Contour", selected_melody_contour, table.concat(melody_contour_options, "\0") .. "\0")
        reaper.ImGui_SameLine(ctx); reaper.ImGui_Text(ctx, "(?)"); if reaper.ImGui_IsItemHovered(ctx) then reaper.ImGui_SetTooltip(ctx, "Sets the overall melodic shape.") end
        
        changed, melody_target_chord_tones = reaper.ImGui_Checkbox(ctx, "Target Chord Tones on Beats", melody_target_chord_tones)
        if reaper.ImGui_IsItemHovered(ctx) then reaper.ImGui_SetTooltip(ctx, "Prioritizes notes from the underlying chord on strong beats.") end
        ...
        changed, selected_arp_strum_pattern = reaper.ImGui_Combo(ctx, "Pattern##Arp", selected_arp_strum_pattern, table.concat(arp_strum_options, "\0") .. "\0")
        ...
        reaper.ImGui_Text(ctx, "Rhythm Applicator")
        changed, selected_rhythm_pattern = reaper.ImGui_Combo(ctx, "Rhythm Pattern", selected_rhythm_pattern, table.concat(rhythm_pattern_options, "\0") .. "\0")
        if reaper.ImGui_Button(ctx, "Apply Rhythm") then apply_rhythm_trigger = true end
        if reaper.ImGui_IsItemHovered(ctx) then reaper.ImGui_SetTooltip(ctx, "Applies the selected rhythmic pattern to the chords.") end
    ...
    if item then
      ...
      if take and reaper.TakeIsMIDI(take) then
        ...
        if #analyzed_chords > 0 then
          ...
          reaper.ShowMessageBox("Could not find enough chords in the item to generate a melody.", "Melody Generation", 0)
        end
