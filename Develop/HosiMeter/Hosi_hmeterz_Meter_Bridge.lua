...
local is_mini_view = false

local layout_defaults = { width = 20, height = 200, group_spacing = 15, meter_spacing = 10 }
local layout_settings = {
  width = layout_defaults.width, height = layout_defaults.height,
  group_spacing = layout_defaults.group_spacing, meter_spacing = layout_defaults.meter_spacing
}
...
local colors = {
  ...
  text            = { 200, 200, 200, 255 }, group_label     = { 255, 200, 100, 255 },
  text_on_meter   = { 240, 240, 240, 255 }, dynamic_range   = { 255, 255, 255,  70 },
  solo_button_on  = { 255, 220,   0, 255 }, solo_text_on    = {   0,   0,   0, 255 },
}
...
function LoadGroupNames()
  ...
  if names_string ~= "" then
    local temp_names = {}
    for name in string.gmatch(names_string, "([^|]*)") do table.insert(temp_names, name) end
    for i = 1, _NGRP do group_names[i] = temp_names[i] or "" end
  else
    for i = 1, _NGRP do group_names[i] = "" end
  end
end

function SaveLayoutSettings()
  local layout_string = table.concat({
    layout_settings.width, layout_settings.height,
    layout_settings.group_spacing, layout_settings.meter_spacing,
    is_mini_view and 1 or 0
  }, "|")
...
function LoadLayoutSettings()
  ...
  if layout_string ~= "" then
    local values = {}
    for val in string.gmatch(layout_string, "([^|]+)") do table.insert(values, tonumber(val)) end
    if #values >= 4 then
    ...
    if #values >= 5 then is_mini_view = (values[5] == 1) end
  end
end
    ...
    for fx_index = 0, num_fx - 1 do
      ...
      if fx_name and string.match(string.lower(fx_name), "hmeterz tx") then
        ...
        if meter_num > 0 then
          ...
          if not track_map[group_idx] then track_map[group_idx] = {} end
...
function loop()
  ...
  if visible then
    if is_mini_view then
        if imgui.IsWindowHovered(ctx) and imgui.IsMouseReleased(ctx, 1) then is_mini_view = false end
        if imgui.SmallButton(ctx, "Full View") then is_mini_view = false end
    ...
    if not is_mini_view then
        if imgui.Button(ctx, "Menu") then imgui.OpenPopup(ctx, "main_menu") end

        if imgui.BeginPopup(ctx, "main_menu") then
          if imgui.MenuItem(ctx, "Reset All Clips & Peaks") then
    ...
    if show_settings and not is_mini_view then
      ...
      if imgui.BeginTable(ctx, "group_names_table", 4) then
        for i = 1, _NGRP do
          ...
          if changed or imgui.IsItemDeactivatedAfterEdit(ctx) then SaveGroupNames() end
          imgui.SameLine(ctx)
      ...
      if changed_width or changed_height or changed_g_space or changed_m_space then
          layout_settings.width = new_width; layout_settings.height = new_height;
          layout_settings.group_spacing = new_g_space; layout_settings.meter_spacing = new_m_space
          SaveLayoutSettings()
      end
      imgui.Separator(ctx)
    end
    ...
    if script_enabled then
      ...
      for g = 0, _NGRP - 1 do
        for n = 1, _NTX do
          ...
          if reaper.gmem_read(gmem_base._TXON + index) == 1 then known_meters[index] = current_time end
        end
      end
      ...
      for index, last_seen in pairs(known_meters) do
        if current_time - last_seen > METER_INACTIVE_TIMEOUT then
          ...
          table.insert(active_data[g].meters, {
            num = n, rms_val = math.sqrt(reaper.gmem_read(gmem_base._RMS + index)),
          ...
          if solo_group_idx == nil or solo_group_idx == group_info.index then
            ...
            if not is_mini_view then
                ...
                if imgui.Button(ctx, "S##" .. g, solo_button_w, 18) then
                  if is_this_group_soloed then solo_group_idx = nil else solo_group_idx = g end
                end
                if is_this_group_soloed then imgui.PopStyleColor(ctx, 4) end
            end
            ...
            for i, meter_data in ipairs(group_info.meters) do
              ...
              if imgui.IsItemClicked(ctx, 0) then -- Left-click
                 ...
                 reaper.gmem_write(gmem_base._MAXPEAK + gmem_index, 0) -- ++ v0.7
